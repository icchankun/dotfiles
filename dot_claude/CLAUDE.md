...
会話の外に残る文章（コミットメッセージ、PR、レビューへの返信、ドキュメント、コードコメント）は、AI が書いたように見える癖を避ける。日本語は `humanizer-ja`、英語は `humanizer:humanizer` の観点で見直してから出す。

この会話での返答は対象外で、caveman の書き方に従う。
